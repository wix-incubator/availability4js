...
availability.DateTimeWindowsIterator = availability.DateTimeWindowsIterator || function(params) { return (function(params) {
	params = params || {};
	...
	self.next = function() {
		if (index.index === timeWindows.length) {
			result = {
				status : availability.STATUS_UNKNOWN,
				until : null
			};
		} else {
			...
			if (!index.isDummyBefore) {
				result = {
					status : (nextTimeWindow.available ? availability.STATUS_AVAILABLE : availability.STATUS_UNAVAILABLE),
